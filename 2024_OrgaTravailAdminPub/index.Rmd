---
title: "Les formes d’organisation du travail dans les administrations publiques : _open science_"
...
res.mca |> 
  ggmca(dat = ct[salariat05, ],
        sup_vars = c("cah_ORGA", "PPP1", "EMP4", "ANNEE", "OBJtous", "RWNORMH", "RWCOLEG"), 
        cah = "cah_ORGA", 
        profiles = TRUE, max_profiles = 5000,
        text_size = 3, text_repel = TRUE, size_scale_max = 6
        scale_color_light = c(material_colors_light()[1:5], rep("black", 6)),
        actives_in_bold = TRUE, sup_in_italic = TRUE
  )
```

```{r graph1, echo=FALSE, fig.width=7, fig.height=7/1.121053, dpi=600}
acm_orga_1_cah <- res.mca |> 
  ggmca(ct[salariat05, ],
        sup_vars = c("cah_ORGA", "PPP1", "EMP4", "ANNEE",
                     "OBJtous", "RWNORMH", "RWCOLEG"), 
        actives_in_bold = TRUE, sup_in_italic = TRUE,
        cah = "cah_ORGA", color_groups = "^.{0}", 
        scale_color_light = c(material_colors_light()[1:5], rep("black", 6)),  # "gray30"
        profiles = TRUE, max_profiles = 5000,
        text_size = 3, text_repel = TRUE,
        xlim = c(-0.93, 1.2), ylim = c(-0.8, 1.1), out_lims_move = TRUE,
        size_scale_max = 6, 
        get_data = TRUE
  )
...
heigth_width_ratio <- (0.93 + 1.2) / (0.8 + 1.1)
...
acm_orga_1_cah_output <-
  ...
  geom_point(
    data = acm_orga_1_cah$profiles_coord, 
    ggplot2::aes(size = wcount, color = color_group),
  ...
  geom_segment(
    data = acm_orga_1_cah_cah |>
      mutate(`Dim 1` = pmin(1.3, pmax(`Dim 1`, -1.03)),
             `Dim 2` = pmin(1.3, pmax(`Dim 2`, -0.85)),
             start1  = pmin(0.95, pmax(`Dim 1`, -0.65)),
  ...
  geom_label(
    data = acm_orga_1_cah_cah |>
      mutate(`Dim 1` = pmin(0.95, pmax(`Dim 1`, -0.65)),
    ...
    fontface = "plain", size = 3, na.rm = TRUE
  )

acm_orga_1_cah_output

# # passer text_size à 3.5
...
```{r graph1, include = FALSE, echo=FALSE}
# Version noir et blanc article
acm_orga_1_cah <- res.mca |> 
  ggmca(ct[salariat05, ],
        sup_vars = c("cah_ORGA", "PPP1", "EMP4", "ANNEE",
                     "OBJtous", "RWNORMH", "RWCOLEG"), 
        actives_in_bold = TRUE, sup_in_italic = TRUE,
        cah = "cah_ORGA", cah_color_groups = "^.{0}", 
        scale_color_light = c("black", rep("grey30", 6)),
        # profiles = TRUE, max_profiles = 5000, size_scale_max = 6, 
        text_size = 3.5, text_repel = TRUE,
        xlim = c(-0.93, 1.2), ylim = c(-0.8, 1.1), out_lims_move = TRUE,
        get_data = TRUE
  )
...
acm_orga_1_cah_output <-
  ...
  ggrepel::geom_text_repel(
    ggplot2::aes(label = lvs, fontface = face),
    size = 3.5, na.rm = TRUE, direction = "both",  min.segment.length = 0.01,
    force = 0.5, force_pull = 1, point.padding = 0, box.padding = 0, 
    point.size = NA, arrow = ggplot2::arrow(length = ggplot2::unit(0.25, "lines"))
  ) +
  geom_segment(
  ...
  geom_label(
    ...
    # direction = "y", force = 0.5, force_pull = 1, point.padding = 0, point.size = NA,
    # arrow = ggplot2::arrow(length = ggplot2::unit(0.25, "lines")),
    fontface = "plain", size = 3.5, na.rm = TRUE
  )
...
<font size=2>
Lecture : en gras, les variables actives (voir détail des intitulés tableau 1) ; encadrées et en couleur les classes de la CAH. En italique les variables supplémentaires : CSP recodée (voir encadré 1 dans l’annexe méthodologique), type d’employeur (voir encadré 2 en annexe), année, objectifs chiffrés (voir tableau 5), contraintes de rythme (voir tableau 1). CR = contraintes de rythme. Les catégories débutant par « Pas » désignent l’absence de quelque chose.
...
cah_orga_subtext <- c(
  ...
  Nuances de bleu : proportion supérieure à la moyenne de l'Ensemble d'au moins 
  5 points de pourcentage. Nuances du jaune au rouge : proportion inférieure 
  à la moyenne d'au moins 5 points de pourcentage.",
  "Lecture : 97 % des salarié·es de la classe « autonomie du métier » 
  ont une autonomie procédurale.",
  champ05, 
  source0519 #, 
)

active_vars_and_sums <- unique(c(active_vars, Ceval, "NBCTREVAL", Cexec, "NBCTREXEC", "NBCONTR"), fromLast = TRUE)

cah_orga_active_tabs <- ct[salariat05, ] |>
  HCPC_tab(row_vars = all_of(active_vars_and_sums), clust = cah_ORGA,
           wt = pondcal, subtext = cah_orga_subtext
  )
...
active_vars_rename_vect <- c(
  ...
  "Part des salarié·es" = "% of population",
  "Nombre d’observations (cumul 2005, 2013 et 2019)" = "n" #,
)

cah_orga_active_tabs <- cah_orga_active_tabs |>
  ungroup() |> 
  mutate(lvs = fct_recode(lvs, !!!active_vars_rename_vect) |>
           fct_relevel(names(active_vars_rename_vect))
  ) |> 
  arrange(variables, lvs) |>
  add_row(variables = factor(""), lvs = factor(
    "Le rythme du travail est imposé par (contraintes de rythme) : "
  ), 
  .after = 17
  ) |> 
  select(-variables) |> 
  mutate(lvs = fct_relabel(lvs, ~ str_wrap(., width = 40) |> 
                             str_replace_all(" ", ggfacto:::unbrk) |>
                             str_replace_all("-", "\\u2011") |> # trait d'union insécable
...
list(
  ...
  # ct[salariat05 & !is.na(ct$EMP4), ] |> group_by(EMP4) |> group_split() %>% #
  #   set_names(map_chr(., ~ pull(., EMP4) |> first() |> as.character())) |> 
  imap_dfr(
...
<!-- Ne pas changer le nom : référence utilisée dans l'article -->
